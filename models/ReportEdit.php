...
class ReportEdit extends Report
{
    ...
    public function run()
    {
        ...
        $this->gambar_terlapor->Visible = false;
        $this->gambar_terproses->setVisibility();
        $this->status_id->setVisibility();
        $this->keterangan->setVisibility();
        $this->tanggal_laporan->Visible = false;
        $this->tanggal_diproses->setVisibility();
    ...
    protected function getUploadFiles()
    {
        ...
        $this->gambar_terproses->Upload->Index = $CurrentForm->Index;
        $this->gambar_terproses->Upload->uploadFile();
        $this->gambar_terproses->CurrentValue = $this->gambar_terproses->Upload->FileName;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'tanggal_diproses' first before field var 'x_tanggal_diproses'
        $val = $CurrentForm->hasValue("tanggal_diproses") ? $CurrentForm->getValue("tanggal_diproses") : $CurrentForm->getValue("x_tanggal_diproses");
        if (!$this->tanggal_diproses->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tanggal_diproses->Visible = false; // Disable update for API request
            } else {
                $this->tanggal_diproses->setFormValue($val);
            }
            $this->tanggal_diproses->CurrentValue = UnFormatDateTime($this->tanggal_diproses->CurrentValue, 1);
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->tanggal_diproses->CurrentValue = $this->tanggal_diproses->FormValue;
        $this->tanggal_diproses->CurrentValue = UnFormatDateTime($this->tanggal_diproses->CurrentValue, 1);
    }
    ...
    protected function newRow()
    {
        ...
        $row['id'] = null;
        $row['lokasi'] = null;
        $row['gambar_terlapor'] = null;
        $row['gambar_terproses'] = null;
        $row['status_id'] = null;
        $row['keterangan'] = null;
        $row['tanggal_laporan'] = null;
        $row['tanggal_diproses'] = null;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->isShow()) {
                RenderUploadField($this->gambar_terproses);
            }
            ...
            if ($this->status_id->ViewValue !== null) { // Load from cache
                ...
                if ($this->status_id->ViewValue == "") {
                    $this->status_id->ViewValue = $Language->phrase("PleaseSelect");
                }
            ...
            $this->tanggal_diproses->EditValue = HtmlEncode(FormatDateTime($this->tanggal_diproses->CurrentValue, 8));
    ...
    protected function validateForm()
    {
        ...
        if ($this->gambar_terproses->Required) {
            if ($this->gambar_terproses->Upload->FileName == "" && !$this->gambar_terproses->Upload->KeepFile) {
                $this->gambar_terproses->addErrorMessage(str_replace("%s", $this->gambar_terproses->caption(), $this->gambar_terproses->RequiredErrorMessage));
            }
        }
        if ($this->status_id->Required) {
            if ($this->status_id->FormValue == "") {
        ...
        if ($this->tanggal_diproses->Required) {
            if (!$this->tanggal_diproses->IsDetailKey && EmptyValue($this->tanggal_diproses->FormValue)) {
                $this->tanggal_diproses->addErrorMessage(str_replace("%s", $this->tanggal_diproses->caption(), $this->tanggal_diproses->RequiredErrorMessage));
            }
        }
        if (!CheckDate($this->tanggal_diproses->FormValue)) {
            $this->tanggal_diproses->addErrorMessage($this->tanggal_diproses->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // gambar_terproses
            if ($this->gambar_terproses->Visible && !$this->gambar_terproses->ReadOnly && !$this->gambar_terproses->Upload->KeepFile) {
                $this->gambar_terproses->Upload->DbValue = $rsold['gambar_terproses']; // Get original value
                if ($this->gambar_terproses->Upload->FileName == "") {
                    $rsnew['gambar_terproses'] = null;
                } else {
                    $rsnew['gambar_terproses'] = $this->gambar_terproses->Upload->FileName;
                }
            }
            ...
            // tanggal_diproses
            $this->tanggal_diproses->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_diproses->CurrentValue, 1), null, $this->tanggal_diproses->ReadOnly);
            if ($this->gambar_terproses->Visible && !$this->gambar_terproses->Upload->KeepFile) {
                $oldFiles = EmptyValue($this->gambar_terproses->Upload->DbValue) ? [] : [$this->gambar_terproses->htmlDecode($this->gambar_terproses->Upload->DbValue)];
                if (!EmptyValue($this->gambar_terproses->Upload->FileName)) {
                    $newFiles = [$this->gambar_terproses->Upload->FileName];
                    $NewFileCount = count($newFiles);
                    for ($i = 0; $i < $NewFileCount; $i++) {
                        if ($newFiles[$i] != "") {
                            $file = $newFiles[$i];
                            $tempPath = UploadTempPath($this->gambar_terproses, $this->gambar_terproses->Upload->Index);
                            if (file_exists($tempPath . $file)) {
                                ...
                                $file1 = UniqueFilename($this->gambar_terproses->physicalUploadPath(), $file); // Get new file name
                                if ($file1 != $file) { // Rename temp file
                                    while (file_exists($tempPath . $file1) || file_exists($this->gambar_terproses->physicalUploadPath() . $file1)) { // Make sure no file name clash
                                        $file1 = UniqueFilename([$this->gambar_terproses->physicalUploadPath(), $tempPath], $file1, true); // Use indexed name
                                    }
                    ...
                    $this->gambar_terproses->Upload->DbValue = empty($oldFiles) ? "" : implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $oldFiles);
                    $this->gambar_terproses->Upload->FileName = implode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $newFiles);
                    $this->gambar_terproses->setDbValueDef($rsnew, $this->gambar_terproses->Upload->FileName, null, $this->gambar_terproses->ReadOnly);
                }
            }
            ...
            if ($updateRow) {
                ...
                if ($editRow) {
                    if ($this->gambar_terproses->Visible && !$this->gambar_terproses->Upload->KeepFile) {
                        ...
                        if (!EmptyValue($this->gambar_terproses->Upload->FileName)) {
                            $newFiles = [$this->gambar_terproses->Upload->FileName];
                            $newFiles2 = [$this->gambar_terproses->htmlDecode($rsnew['gambar_terproses'])];
                            $newFileCount = count($newFiles);
                            for ($i = 0; $i < $newFileCount; $i++) {
                                if ($newFiles[$i] != "") {
                                    $file = UploadTempPath($this->gambar_terproses, $this->gambar_terproses->Upload->Index) . $newFiles[$i];
                                    if (file_exists($file)) {
                                        if (@$newFiles2[$i] != "") { // Use correct file name
                                            $newFiles[$i] = $newFiles2[$i];
                                        }
                                        if (!$this->gambar_terproses->Upload->SaveToFile($newFiles[$i], true, $i)) { // Just replace
        ...
        if ($editRow) {
            // gambar_terproses
            CleanUploadTempPath($this->gambar_terproses, $this->gambar_terproses->Upload->Index);
        }
